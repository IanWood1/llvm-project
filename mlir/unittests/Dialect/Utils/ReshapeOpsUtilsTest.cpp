...
TEST(ReassociationIndicesForCollapse, DynamicTest) {
  ...
  //===----------------------------------------------------------------------===//
  // TODO: the reassociation for the following examples are possible to compute
  // but are limited by `getReassociationIndicesForCollapse`.
  //===----------------------------------------------------------------------===//

  EXPECT_NE(getReassociationIndicesForCollapse({ShapedType::kDynamic, 10},
                                               {ShapedType::kDynamic}),
            makeOptionalIndices({{0, 1}}));
  EXPECT_NE(getReassociationIndicesForCollapse(
                {1, ShapedType::kDynamic, ShapedType::kDynamic},
                {ShapedType::kDynamic}),
            makeOptionalIndices({{0, 1, 2}}));
  EXPECT_NE(getReassociationIndicesForCollapse({10, ShapedType::kDynamic},
                                               {ShapedType::kDynamic}),
            makeOptionalIndices({{0, 1}}));
  EXPECT_NE(getReassociationIndicesForCollapse({ShapedType::kDynamic, 10, 20},
                                               {ShapedType::kDynamic, 20}),
            makeOptionalIndices({{0, 1}, {2}}));
  EXPECT_NE(getReassociationIndicesForCollapse({10, ShapedType::kDynamic, 20},
                                               {ShapedType::kDynamic, 20}),
            makeOptionalIndices({{0, 1}, {2}}));
  EXPECT_NE(getReassociationIndicesForCollapse(
  ...
  EXPECT_NE(getReassociationIndicesForCollapse({1, ShapedType::kDynamic, 1},
                                               {ShapedType::kDynamic}),
            makeOptionalIndices({{0, 1, 2}}));
  EXPECT_NE(getReassociationIndicesForCollapse(
                {1, ShapedType::kDynamic, ShapedType::kDynamic},
...
TEST(ReassociationIndicesForCollapse, DynamicTestFailure) {
  ...
  // TODO(#137963): This should be `EXPECT_EQ`. This is a bug in
  // `getReassociationIndicesForCollapse`.
  EXPECT_NE(
      getReassociationIndicesForCollapse(
          {ShapedType::kDynamic, ShapedType::kDynamic, ShapedType::kDynamic},
